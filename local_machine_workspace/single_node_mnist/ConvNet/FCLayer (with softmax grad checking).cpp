...
void FCLayer::ComputeSoftmaxGradientsT(float *h_labels) {
  ...
  elapsed = std::chrono::duration_cast<std::chrono::microseconds>(et - st);

  //print_d_var(d_softmax_gradients_T, output_neurons, input_neurons + 1);

  float *h_sg = (float *)malloc(sizeof(float) * (input_neurons + 1)
                                * output_neurons);
  float *h_sg_T = (float *)malloc(sizeof(float) * (input_neurons + 1)
                                * output_neurons);

  cudaMemcpy(h_sg, d_softmax_gradients, sizeof(float) * (input_neurons + 1)
             * output_neurons, cudaMemcpyDeviceToHost);
  cudaMemcpy(h_sg_T, d_softmax_gradients_T, sizeof(float) * (input_neurons + 1)
             * output_neurons, cudaMemcpyDeviceToHost);
  //h_sg_T[3] += 1e-3f;
  for (int i = 0; i < input_neurons + 1; i++) {
    for (int j = 0; j < output_neurons; j++) {
      if ((h_sg[j + i * output_neurons]
          - h_sg_T[i + j * (input_neurons + 1)] > 1e-5f)
          || (h_sg_T[i + j * (input_neurons + 1)]
              - h_sg[j + i * output_neurons] > 1e-5f)) {
        std::cout << "Mismatch!";
        std::cout << h_sg[j + i * output_neurons] << ", "
          << h_sg_T[i + j * (input_neurons + 1)] << std::endl;
      }
    }
    std::cout << std::endl;
  }
